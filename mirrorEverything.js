...
var mirrorDir = -1; // 0 = X, 1 = Y, 2 = Z    10 = Swap all words "right" to "left" and vice-versa
...
function mirrorleftright(json) {
  msg("Progress: Swapping words \"Left\" and \"Right\"");
  if(json == "") {
    msg("Progress: Aborted, no json", "abort");
    details("User is not editing an object, Aborted");
  } else {
      ...
      for(statei = 0; statei < JSONobj["p"][parti]["s"].length; statei++) {
        ...
        if(JSONobj["p"][parti]["s"][statei].hasOwnProperty("b")) {
          for(scripti = 0; scripti < JSONobj["p"][parti]["s"][statei]["b"].length; scripti++) {
            JSONobj["p"][parti]["s"][statei]["b"][scripti] = swapLeftRight(JSONobj["p"][parti]["s"][statei]["b"][scripti]);
          }
        }
      }
    }
...
function swapLeftRight(scriptLine) {
  scriptLine = scriptLine.replace(/left/g, "RIGHT").replace(/right/g, "left").replace(/RIGHT/g, "right");
  return scriptLine;
}
